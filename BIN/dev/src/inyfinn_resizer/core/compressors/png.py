...
def png_max_colors_for_quality(quality_pct: int) -> int:
    """Mapuje suwak jakości (0–100) na liczbę kolorów palety PNG.

    Kalibracja Photoshop (PNG-8): 50% → 160 kolorów, ≤10% → 24 kolory.
    """
    q = max(5, min(100, int(quality_pct)))
    if q <= 10:
        return MIN_PALETTE_COLORS
    if q >= 50:
        return int(round(160 + (q - 50) * (MAX_PALETTE_COLORS - 160) / (100 - 50)))
...
def count_rare_green_accents(path: Path) -> int:
    """Liczy rzadkie zielone akcenty (np. zioła na potrawie)."""
    try:
    ...
    try:
        with Image.open(path) as img:
            ...
            pixels = img.getdata()
            return sum(1 for r, g, b in pixels if g > r + 12 and g > b + 12 and g > 80)
    except OSError:
        return 0


def accents_preserved(src: Path, dst: Path, *, quality_pct: int = 100) -> bool:
    """Sprawdza, czy rzadkie zielone akcenty przetrwały kompresję."""
    src_green = count_rare_green_accents(src)
    if src_green == 0:
        return True
    dst_green = count_rare_green_accents(dst)
    q = max(5, min(100, int(quality_pct)))
    if q <= 50:
        min_ratio = 0.2
    elif q <= 70:
        ...
        min_ratio = 0.45 if src_green <= 20 else 0.6
    if src_green <= 20:
        return dst_green >= max(1, int(src_green * min_ratio))
    return dst_green >= int(src_green * min_ratio)


def scale_accent_boost(boost: int, quality_pct: int) -> int:
    """Przy niskiej jakości nie podbijaj agresywnie palety — inaczej brak kompresji."""
    q = max(5, min(100, int(quality_pct)))
    factor = max(0.15, q / 100.0)
    return int(round(boost * factor))


def png_auto_target_kb(source_bytes: int, quality_pct: int) -> float:
    """Szacowany rozmiar docelowy z suwaka (50% ≈ 25% rozmiaru źródła)."""
    q = max(5, min(100, int(quality_pct)))
    source_kb = source_bytes / 1024.0
    ratio = max(0.12, q / 200.0)
    return max(48.0, source_kb * ratio)


def has_sensitive_accents(path: Path) -> bool:
    return analyze_accent_palette_boost(path) >= 16 or count_rare_green_accents(path) > 0
...
def analyze_accent_palette_boost(path: Path) -> int:
    ...
    try:
        ...
        rare_green = sum(
            1
            for i in range(0, len(pixels), step)
            if (r := pixels[i][0]) is not None
            and (g := pixels[i][1]) > r + 12
            and g > (b := pixels[i][2]) + 12
            and g > 80
        )

        boost = 0
        # Nie podbijaj palety, gdy cały obraz jest nasycony (np. cynamon, baton).
        # _compress_wizek.py / PS: 50% = dokładnie 160 kolorów bez boostu.
        uniform_saturation = sat_ratio >= 0.35
        if not uniform_saturation:
            if distinct_hues >= 10 or sat_ratio >= 0.08:
                ...
                boost = 16
        if rare_green > 0 and rare_green / samples < 0.02:
            boost = max(boost, 48)
        return boost
    except OSError:
        return 0
...
def effective_png_palette(path: Path, base_palette: int, *, quality_pct: int = 100) -> int:
    boost = scale_accent_boost(analyze_accent_palette_boost(path), quality_pct)
    return max(
        MIN_PALETTE_COLORS,
        min(MAX_PALETTE_COLORS, int(base_palette) + boost),
    )
...
def pngquant_params_for_quality(
    ...
    *,
    ...
    return (max(0, qmin), min(100, qmax), speed)
...
def run_pngquant(
    ...
    *,
    ...
    Dithering Floyd-Steinberg jest domyślnie WŁĄCZONY (pngquant nie ma flagi --floyd).
    Jedyna opcja związana z ditheringiem to --nofs (wyłączenie). Używamy jej tylko
    przy preserve_accents, żeby chronić rzadkie plamy koloru.
    """
    palette = max(MIN_PALETTE_COLORS, min(MAX_PALETTE_COLORS, int(max_colors)))
    # pngquant: [options] [ncolors] -- pngfile  (ncolors = opcjonalny argument pozycyjny, nie --colors)
    cmd = [
        str(pngquant),
        "--force",
...
def apply_pngquant(
    ...
    *,
    ...
    accent_boost = analyze_accent_palette_boost(path)
    palette = effective_png_palette(path, base_palette, quality_pct=quality_pct)
    accent_guard = has_sensitive_accents(path) and quality_pct >= 70
